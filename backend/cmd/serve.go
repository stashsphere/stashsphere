...
func SetupWithDB(db *sql.DB, config config.StashSphereServeConfig, debug bool, serveOpenAPI bool, openAPIPath string) (*echo.Echo, *fuego.Engine, error) {
	...
	fuegoecho.GetEcho(engine, thingsGroup, "", thingHandler.ThingHandlerIndex,
		...
		option.QueryInt("page", "Page number for pagination (0-indexed)", param.Example("page 0", 0)),
		...
		option.Query("searchTerm", "Search term to filter things by name", param.Example("search", "hammer")),
		option.QueryBool("paginate", "Enable pagination (default: true)", param.Example("paginate", true)),
		// fuego does not support this yet param.Enum("access-reason:desc", "created-at:desc")),
	...
	fuegoecho.GetEcho(engine, imageGroup, "", imageHandler.ImageHandlerIndex,
		...
		option.QueryInt("page", "Page number for pagination (0-indexed)", param.Example("page 0", 0)),
		option.QueryInt("perPage", "Items per page (default: 50)", param.Example("50 items", 50)),
		option.QueryBool("onlyUnassigned", "Filter to show only unassigned images", param.Example("only unassigned", true)),
		option.AddResponse(
			200,
	...
	fuegoecho.GetEcho(engine, notificationsGroup, "", notificationHandler.Index,
		...
		option.QueryInt("page", "Page number for pagination (0-indexed)", param.Example("page 0", 0)),
		option.QueryInt("perPage", "Items per page (default: 50)", param.Example("50 items", 50)),
		option.QueryBool("onlyUnacknowledged", "Filter to show only unacknowledged notifications", param.Example("only unacknowledged", true)),
		option.AddResponse(
			200,
