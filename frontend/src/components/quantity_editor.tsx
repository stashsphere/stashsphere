...
const QuantityEditor = ({ quantity, unit, onChange }: Props) => {
    ...
    return <div className="flex flex-col">
        <div className="flex flex-row text-display mb-4">
            <button className={`${buttonClasses} rounded-l-full`} onClick={() => onQuantityChange(quantity - 1)}>
                <Icon icon="mdi--minus" />
            </button>
            <input className={`w-16 ${hideArrows} text-right px-2`} type="number" min={0} step={1} onChange={(e) => onQuantityChange(Number(e.target.value))} value={quantity} />
            <button className={`${buttonClasses} rounded-r-full`} onClick={() => onQuantityChange(quantity + 1)}>
                <Icon icon="mdi--plus" />
            </button>
        ...
        <div className="">
            ...
            <input
                ...
                className="w-32 mt-1 p-2 text-display border border-gray-300 rounded"
            />
        </div>
    </div>
        ;
