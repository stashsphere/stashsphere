    ...
    {targetUserProfile === null ?
      <>
        <div className="relative flex items-center my-2">
          ...
          <input onChange={(e) => setSearchTerm(e.target.value)} value={searchTerm} type="text" placeholder="Search for names and email addresses" className="block w-full py-2.5 text-gray-700 placeholder-gray-400/70 bg-white border border-gray-200 rounded-lg pl-11 pr-5 rtl:pr-11 rtl:pl-5 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40">
          </input>
        </div>
        <ProfileList profiles={selectableProfiles} hintText="Share to this user" onClick={setTargetUserProfile} /></> :
      <>
        <div className="flex items-center p-3 mt-2 text-sm text-gray-600 dark:text-gray-300 border border-green-500 mb-2">
          ...
          <div className="mx-1">
            <h1 className="text-sm font-semibold text-gray-700 dark:text-gray-200">{targetUserProfile.name}</h1>
        ...
        <form onSubmit={onSubmit}>
          <PrimaryButton
            type="submit"
          >
            Share to {targetUserProfile.name}
          </PrimaryButton>
        </form>
      </>
    }
